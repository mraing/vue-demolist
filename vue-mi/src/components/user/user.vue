<template>
  <div id="user" class="user">
    <header class="user-header">
      <!-- 头像 -->
      <div class="user-header-photo">
        <img src="https://m.mi.com/static/img/avatar.76a75b8f17.png">
      </div>
      <div class="user-header-name">登录/注册</div>
    </header>

    <!-- 我的订单 -->
    <div class="user-order">
      <a class="user-order-all">
        <span>我的订单</span>
        <span>
          全部订单
          <i class="iconfont icon-iconRight"></i>
        </span>
      </a>
      <div class="user-order-common">
        <a class="user-order-common-link">
          <span class="iconfont icon-daifukuan"></span>
          <span>待付款</span>
        </a>
        <a class="user-order-common-link">
          <span class="iconfont icon-truck"></span>
          <span>待收货</span>
        </a>
        <a class="user-order-common-link">
          <span class="iconfont icon-banshou"></span>
          <span>退换修</span>
        </a>
      </div>
    </div>

    <!-- 操作列表 -->
    <!-- <div class="user-option-list">
      <a class="user-option-items">
        <div class="user-option-items-left"  style="background:url('https://m.mi.com/static/img/i-setting.fb9625b3f2.png') 12.5px 50% / 25px no-repeat">会员中心</div>
        <div class="user-option-items-right">
          <i class="iconfont icon-iconRight"></i>
        </div>
      </a>
    </div> -->

    <div class="user-option-list" v-for="(item,index) in optionList" :key="index">
      <a class="user-option-items" v-for="(citem,index) in item" :key="index">
        <div class="user-option-items-left" :style="{background: 'url(' + citem.iconImg + ') 12.5px 50% / 25px no-repeat'}">{{citem.title}}</div>
        <div class="user-option-items-right">
          <i class="iconfont icon-iconRight"></i>
        </div>
      </a>
    </div>

  </div>
</template>

<script>
export default {
  data() {
    return {
      title: "user",
      optionList: [
        [
          {
            iconImg:
              "data:image/png;base64,",
            title: "会员中心",
            urlLink: ""
          },
          {
            iconImg:
              "data:image/png;base64,",
            title: "我的优惠",
            urlLink: ""
          }
        ],

        [
          {
            iconImg:
              "data:image/png;base64,",
            title: "服务中心",
            urlLink: ""
          },
          {
            iconImg:
              "data:image/png;base64,",
            title: "小米之家",
            urlLink: ""
          }
        ],
        [
          {
            iconImg: "data:image/png;base64,",
            title: "F码通道",
            urlLink: ""
          }
        ],
        [
          {
            iconImg: "https://m.mi.com/static/img/i-setting.fb9625b3f2.png",
            title: "设置",
            urlLink: ""
          }
        ]
      ]
    };
  }
};
</script>



<style>
.user{
  padding-bottom: 70px;
  background: #f5f5f5;
}
.user-header {
  display: flex;
  padding: 20px 10px;
  background: url(https://m.mi.com/static/img/bg.63c8e19851.png) center 0 #f37d0f;
  background-size: auto 100%;
}
.user-header-photo {
  width: 50px;
  height: 50px;
}
.user-header-photo > img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
.user-header-name {
  padding-left: 10px;
  font-size: 14px;
  color: #fff;
  line-height: 50px;
}
.user-order{
  background: #fff;
}
.user-order-all {
  display: flex;
  justify-content: space-between;
  padding: 0 10px;
  height: 40px;
  border-bottom: 1px solid rgba(0, 0, 0, 0.15);
}
.user-order-all > span {
  font-size: 12px;
  color: #2c2c2c;
  line-height: 40px;
  text-decoration: none;
}
.user-order-all > span > i {
  padding-left: 5px;
  font-size: 12px;
}
.user-order-common {
  padding: 0 10px;
  display: flex;
  justify-content: space-around;
  border-bottom: 10px solid #f5f5f5;
}
.user-order-common-link {
  display: block;
  padding: 10px 0;
}
.user-order-common-link > span {
  display: block;
  font-size: 12px;
  color: rgba(0, 0, 0, 0.87);
  line-height: 28px;
}
.user-order-common-link > span:first-child {
  font-size: 20px;
  color: #999;
}
.user-option-items {
  display: flex;
  justify-content: space-between; 
  padding: 0 10px;
  height: 50px;
  line-height: 50px;
  border-bottom: 1px solid #eee;
}
.user-option-items:last-child {
  border-bottom: none;
}
.user-option-items-left {
  padding-left: 50px;
}
.user-option-list{
  /* margin-bottom: 10px; */
  background: #fff;
  border-bottom: 10px solid #f5f5f5;
}

</style>

